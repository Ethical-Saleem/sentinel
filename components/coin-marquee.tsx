...
export default function CoinMarqueeWidget() {
  ...
  const coinCategories = {
    spot: ['bitcoin', 'ethereum', 'tether', 'binancecoin', 'solana', 'ripple', 'usd-coin', 'cardano', 'avalanche-2', 'dogecoin', 'tron', 'polkadot', 'polygon', 'wrapped-bitcoin', 'shiba-inu', 'dai', 'litecoin', 'bitcoin-cash', 'chainlink', 'uniswap'],
    margin: ['bitcoin', 'ethereum', 'binancecoin', 'solana', 'ripple', 'cardano', 'avalanche-2', 'polkadot', 'polygon', 'litecoin', 'bitcoin-cash', 'chainlink', 'uniswap', 'stellar', 'ethereum-classic', 'filecoin', 'monero', 'cosmos', 'aptos', 'near'],
    staking: ['ethereum', 'cardano', 'solana', 'polkadot', 'avalanche-2', 'polygon', 'cosmos', 'near', 'algorand', 'tezos', 'kava', 'flow', 'harmony', 'terra-luna-2', 'osmosis', 'fantom', 'elrond-erd-2', 'the-graph', 'pancakeswap-token', 'aave'],
  };
      ...
      <div className="flex-shrink-0 bg-gray-800/50 flex flex-row items-center rounded-lg py-2.5 px-3 mx-2 h-[66px] border border-gray-700/50 hover:border-purple-500/50 transition-all">
        <div className="flex flex-col items-start justify-center gap-0.5">
          <div className="flex flex-row items-center gap-2">
            <img src={coin.image} alt={coin.name} className="w-[20px] h-[20px] rounded-full" />
            <div className="flex flex-row items-center gap-1">
              <div className="font-semibold text-white text-[0.875rem]">{coin.symbol.toUpperCase()}</div>
          ...
          <div className="flex flex-row items-center gap-1">
            ...
            <div className={`text-[0.875rem] font-medium ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
  ...
  const MarqueeRow = ({ coins, duration }: { coins: any[], duration: number }) => {
